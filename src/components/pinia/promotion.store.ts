...
export const usePromotionStore = defineStore('promotion', () => {
  ...
  const sortColumn = ref('promotionName')
  ...
  const fetchPromotions = async (page: number) => {
    try {
      const response = await fetch(`http://localhost:3000/api/promotion?page=${page}&limit=${itemsPerPage}&sortColumn=${sortColumn.value}&sortOrder=${sortOrder.value}`)
  ...
  const addPromotion = async (newPromotion: Omit<Promotion, 'id'> & { promotionType: 'invoice' | 'dish', dishes?: string[], minimumSpend?: number, promotionLimit?: number }) => {
    try {
