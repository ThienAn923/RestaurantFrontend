...
const navigateToPrintPage = () => {
  window.open(router.resolve({ path: '/printInvoice', query: { invoice: JSON.stringify(currentInvoice.value) } }).href, '_blank');
};
</script>
...
<template>
  <div class="h-full w-full bg-gray-50 overflow-auto p-6">
    ...
    <Dialog v-model:open="isDetailModalOpen">
      <DialogContent class="sm:max-w-[210mm] h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Chi Tiết Hóa Đơn</DialogTitle>
          <DialogDescription>
            Thông tin đầy đủ cho hóa đơn được in dưới đây. Để in hóa đơn, nhấn vào nút "In Hóa Đơn".
          </DialogDescription>
        </DialogHeader>
                  ...
                  <Table v-if="currentInvoice.invoiceDetails.length > 0">
                    ...
                    <TableBody>
                      <TableRow v-for="detail in currentInvoice.invoiceDetails" :key="detail.id">
                        ...
                        <TableCell class="font-bold text-gray-600">{{ formatCurrency(detail.totalCost) }}</TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
        ...
        <div class="mt-4">
          <Button @click="navigateToPrintPage">In Hóa Đơn</Button>
        </div>
      </DialogContent>
    </Dialog>
  </div>
