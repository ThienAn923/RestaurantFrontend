...
const props = withDefaults(defineProps<BaseChartProps<T> & {
  ...
  showGridLine: true,
  showGradiant: true,
  totals: { income: 999, expense: 999, ingredientCost: 999 }
})
...
const legendItems = ref<BulletLegendItemInterface[]>(props.categories.map((category, i) => ({
  ...
  inactive: false,
  // totals: props.totals ? props.totals[category as keyof typeof props.totals] : 0 //custom
})))
...
console.log("totals from areChart", props.totals);
</script>

<template>
  <div :class="cn('w-full h-[400px] flex flex-col items-end', $attrs.class ?? '')">
    <ChartLegend v-if="showLegend" v-model:items="legendItems" @legend-item-click="handleLegendItemClick"
      :totals="props.totals" /> <!-- total was custome -->
    <VisXYContainer :style="{ height: isMounted ? '100%' : 'auto' }" :margin="{ left: 20, right: 20 }" :data="data">
      <svg width="0" height="0">
        <defs>
          <linearGradient v-for="(color, i) in colors" :id="`${chartRef}-color-${i}`" :key="i" x1="0" y1="0" x2="0"
            y2="1">
      ...
      <template v-for="(category, i) in categories" :key="category">
        <VisLine :x="(d: Data, i: number) => i" :y="(d: Data) => d[category]" :color="colors[i]" :curve-type="curveType"
          :attributes="{
            [Line.selectors.line]: {
              opacity: legendItems.find(item => item.name === category)?.inactive ? filterOpacity : 1,
            },
